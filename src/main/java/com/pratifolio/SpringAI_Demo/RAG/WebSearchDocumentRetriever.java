...
public class WebSearchDocumentRetriever implements DocumentRetriever {
    ...
    public WebSearchDocumentRetriever(RestClient.Builder restClietBuilder, int resultLimit) {

        Assert.notNull(restClietBuilder, "RestClient cannot be null");
        String apiKey = System.getenv(TAVILY_API_KEY);
        Assert.hasText(apiKey, "Environment variable " + TAVILY_API_KEY + " must be set");
        this.restClient = restClietBuilder
                .baseUrl(TAVILY_BASE_URL)
                .defaultHeader(HttpHeaders.AUTHORIZATION, "Bearer " + apiKey)
                .build();

        if (resultLimit <= 0) {
            throw new IllegalArgumentException("Tavily Search Result Limit must be greater than 0");
        }
